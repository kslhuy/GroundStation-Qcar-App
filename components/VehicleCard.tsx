...
export const VehicleCard: React.FC<VehicleCardProps> = ({
  ...
  const getStateColor = (state?: string) => {
    ...
    if (upper.includes('INIT')) return 'text-cyan-400';
  ...
  return (
    <div
      onClick={onSelect}
      className={`relative p-3 rounded-none-none border transition-all cursor-pointer overflow-hidden
        ${isSelected
          ? 'bg-[#111] border-cyan-500 shadow-lg shadow-cyan-900/20'
          : 'bg-[#111] border-white/20 hover:border-slate-600'
        }
      `}
    >
      {/* Header */}
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2 flex-1">
          <div className={`w-2 h-2 rounded-none-none animate-pulse ${isOnline ? 'bg-green-500' : 'bg-slate-500'}`} />
          {isEditingName ? (
            <div className="flex items-center gap-1 flex-1">
              <input
                ...
                className="bg-[#222] text-slate-100 px-2 py-0.5 rounded-none text-sm font-semibold flex-1 max-w-[100px] focus:outline-none focus:ring-1 focus:ring-cyan-500"
                autoFocus
                onClick={(e) => e.stopPropagation()}
              />
              <button
                onClick={(e) => { e.stopPropagation(); handleNameSave(); }}
                className="p-1 hover:bg-[#222] rounded-none"
              >
            ...
            <h3
              className="font-semibold text-slate-100 text-sm cursor-pointer hover:text-cyan-400 transition-colors flex items-center gap-1 group"
        ...
        {isOnline ? <Wifi size={14} className="text-green-500" /> : <WifiOff size={14} className="text-gray-500" />}
      </div>
      ...
      <div className="flex justify-between items-start mb-2">
        ...
        {isOnline && (vehicle.telemetry.platoon_enabled || globalPlatoonPosition) && (
          <div className={`text-[9px] px-1.5 py-0.5 rounded-none font-bold whitespace-nowrap ${(vehicle.telemetry.platoon_is_leader ?? isGlobalLeader) ? 'bg-amber-900/50 text-amber-400 border border-amber-500/30' : 'bg-blue-900/50 text-cyan-400 border border-cyan-500/30'}`}>
      ...
      {/* Offline Overlay */}
      {!isOnline && (
        <div className="absolute inset-0 bg-[#0a0a0a] backdrop-blur-[1px] flex items-center justify-center z-10">
